(function($) {
    ...
    var blog = {
        ...
        add: function(response) {
            $('#blog-section .blog-content .load-more').transition({
                opacity: 0
            }, 400, function() {
                $(this).remove();
            });
    ...
    var portfolio = {
        ...
        changeTo: function(url) {
            ...
            items.each(function(i, el) {
                $(el).delay(removeTime * i).transition({
                    opacity: 0
                }, function() {
                    if (i === itemsLength) {
        ...
        overlay: {
            ...
            toggleFullscreen: function() {
                ...
                if (this.isLarge === false && this.isAnimating === false) {
                    ...
                    $('.portfolio-overlay .gallery').transition({
                        opacity: 0
                    }, function() {
                    ...
                    $(".portfolio-overlay article").transition({
                        x: '-100%'
                    }, function() {
                        that.Animating = false;
                    });
                    $('.controls2').transition({
                        opacity: 0
                    }).removeClass('hidden').transition({
                        opacity: 1
                    });
                    $('.hover-fullscreen').addClass('hidden');
                    $('.hover-normal-size, .close-overlay').transition({
                        opacity: 0
                    }).removeClass('hidden').transition({
                        opacity: 1
                    }, function() {
    ...
    var sharer = {
        ...
        open: function(url, data) {
            ...
            $('.share-overlay').transition({
                x: 0,
                opacity: 0.9
            }).addClass('overlay-active-share');
        },
        close: function() {
            $('.overlay').transition({
                x: -90,
    ...
    $('.home').on('click', '#close-subnav', function() {
        ...
        $('.menu-' + direction + ' .nav-wrapper').transition({
            opacity: 0
        });
        $('.home').data('direction', 'center').removeClass('subnav-is-active');
        $('.sub-nav.is-active').transition({
            opacity: 0
        }, function() {
    ...
    function openSubnav(direction) {
        if ($('.home').data('direction') !== direction) {
            ...
            $('.menu-' + direction + ' .nav-wrapper').delay(400).transition({
                opacity: 1
            });
        } else {
            // close nav
    ...
    $(document).ready(function() {
        if (!mobile) {
            if (!ipad) {
                $('.section-scroll, .full-post').niceScroll({
                    horizrailenabled: false
                }).resize();
                $('#team .team-info-wrapper').niceScroll({
                    horizrailenabled: false,
                    railalign: "left"
                }).resize();
            }
    ...
    $('.reorder a').click(function(e) {
        ...
        if ($('body').hasClass('mobile-nav-show')) {
            ...
            $('#flyout-container').animate({
                height: $('#flyout-container .flyout-menu > li').height() * $('#flyout-container .flyout-menu > li').length
            }, function() {
    ...
    $('.flyout-menu .open-children').click(function(e) {
        ...
        if ($(this).next('.subnav').length > 0) {
            ...
            if ($(this).next('.subnav').hasClass('open')) {
                ...
                $(this).next('.subnav').animate({
                    height: $(this).next('.subnav').children('li').height() * $(this).next('.subnav').children('li').length
                }, function() {
        ...
        function init() {
            ...
            if (currentUrl['p'] !== undefined) {
                if (currentUrl['p'] !== 'home') {
                    ...
                    historyApi.replace('home.html', '', '#home', 'move');
                    sectionManager.load('#home');
                }
            } else {
                historyApi.replace('home.html', '', '#home', 'move');
            ...
            fadeTransition: function(orientation, complete) {
                ...
                $('.progressbar').animate({
                    ...
                    start: function() {
                        $('.progressbar').css({
                            width: '100%',
                            transition: 'width ' + that.options.play + 'ms linear'
                        });
                    }
                }
                );
